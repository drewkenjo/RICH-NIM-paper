...
The communication between FPGA board and PC is performed using TCP/IP protocol over optical Ethernet (1000BASE-SX).
%The HV cables, one per each module, supply power for attached MAPMTs.
The Data Acquisition program executes on remote PC running Linux OS, configures FPGA and MAROC boards and collects the data through network interface.
...
The current setup allows fast evaluation of FE modules with highly automatized procedure which is important because RICH panel consists of 115 tiles with 3-MAPMTs and 23 tiles with 2-MAPMTs FE modules.
